...
export function PaymentSettings() {
  ...
  function handleScrollToStripeConnect() {
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      form.setValue("acceptOnlinePayments", true);
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [settings, currentStripeStatus] = await Promise.all([
          getPaymentSettings(),
        ...
        if (currentStripeStatus) {
          setStripeStatus({
            status: currentStripeStatus.details_submitted
              ? "active"
              : "pending",
            ...currentStripeStatus,
          });
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: message || "Failed to load payment settings",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleConnectStripe = async () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: message || "Failed to connect Stripe account",
      });
      setConnecting(false);
    }
  };
  ...
  const handleDisconnectStripe = async () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: message || "Failed to disconnect Stripe account",
      });
    } finally {
  ...
  const handleOpenDashboard = async () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: message || "Failed to open Stripe dashboard",
      });
    } finally {
      setConnecting(false);
    }
  ...
  const onSubmit = async (values: z.infer<typeof paymentSettingsSchema>) => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: message || "Failed to update payment settings",
      });
    }
  };
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {!stripeStatus.details_submitted && (
                            <span>
                              Connect your{" "}
                              <span
                                className="underline text-hairsby-orange/90 cursor-pointer"
                                onClick={() => handleScrollToStripeConnect()}
                              >
                                stripe account{" "}
                              </span>
                              below to start taking card payment.{" "}
                            </span>
                          )}
                        </p>
                      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2" ref={stripeRef}>
              {stripeStatus.status === "not_connected" ? (
                <Button
