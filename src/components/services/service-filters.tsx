...
export function ServiceFilters({
  ...
  const debouncedDurationChange = useDebouncedCallback(
    (value: number[]) => onDurationChange([value[0], value[1]]),
    500
  );
        ...
        <div className="space-y-2">
          <button
            ...
            className={`block text-sm ${
              selectedCategory === ""
                ? "font-medium text-hairsby-orange"
                : "text-gray-600"
            }`}
          >
            All Categories
          ...
          {categories?.length > 0 &&
            categories.map((category) => (
              ...
              >
                {category.name}
                {category?.serviceCount && (
                  <span className="ml-1">({category.serviceCount})</span>
                )}
              </button>
            ))}
        </div>
      ...
      <div>
        ...
        <div className="px-2">
          <Slider
            ...
            onValueCommit={debouncedDurationChange}
            className="my-4"
          />
          <div className="flex justify-between text-sm text-gray-600">
            <span>{durationRange[0]} min</span>
